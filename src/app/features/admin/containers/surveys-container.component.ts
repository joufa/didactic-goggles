...
import { fetchSurveys, addSurvey, selectSurvey } from '../actions/survey.actions';
@Component({
    ...
    <agf-survey-list [loading]="surveysLoading$ | async" [error]="surveysError$ | async" [surveys]="surveys$ | async" (edit)="select($event)"></agf-survey-list>
    `,
    styles: [
        `
          .title-card {
